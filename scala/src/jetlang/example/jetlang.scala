...
object DownloadActor extends JetlangActor with ThreadedFiber {
  def act() = {
...
object IndexActor extends JetlangActor with ThreadedFiber {
  def act() = {
    case Message(id, payload) => {
      //println("Indexed " + id)
      WriteActor !
...
object WriteActor extends JetlangActor with ThreadedFiber {
  def act() = {
    case Message(id, payload) => {
      //println("Wrote " + id)
    }
